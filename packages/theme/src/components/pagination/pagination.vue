<template>
  <ul class="pagination">
    <li class="d-flex align-items-center me-2">
      <span class="mx-1">共<span class="text-primary mx-1" v-text="total"></span>条 </span>
      <span>第</span>
      <input class="form-control mx-1" style="height: 20px; width: 32px" value="1" />
      <span>/<span class="text-primary mx-1" v-text="pageCount"></span>页</span>
    </li>
    <li v-for="page in pagers" :key="page" class="page-item" :class="{ active: page.active, disabled: page.disabled }">
      <a class="page-link" @click="onPagerClick(page.page)" v-html="page.text || page.page"></a>
    </li>
    <li class="ms-1">
      <Dropdown>
        <a class="page-link dropdown-toggle">{{ pageSize }}条/页</a>
        <DropdownMenu class="dropdown-menu-end">
          <a v-for="text in pageSizes" :key="text" class="dropdown-item" v-text="text"></a>
        </DropdownMenu>
      </Dropdown>
    </li>
  </ul>
...
export default {
  ...
  props: {
    ...
    pageSizes: { type: Array, default: () => [20, 40, 60, 80, 100] }, //每页显示条目个数
  ...
  setup(props) {
    const pagers = computed(() => {
      ...
      function addPagers(step, opts = []) {
        ...
        const left: any = { page: pagerStart }
      ...
      // //add next
      addPagers(1, ['<i class="fa fa-angle-left"></i>', '<i class="fa fa-angle-right"></i>'])
